// src/pages/EditCategoryForm.jsx
...
const EditCategoryForm = () => {
  ...
  const [file, setFile] = useState(null);   // new icon (optional)
  const [existingIcon, setExistingIcon] = useState(""); // current icon URL
    ...
    const loadCategory = async () => {
      try {
        const res = await fetch(
          `http://localhost:5000/api/categories?category_id=${id}`
        );
        const result = await res.json();

        // Backend returns { success: true, data: [...] } even for single item
        if (result.success && result.data && result.data.length > 0) {
          const cat = result.data[0]; // first (and only) item
          setName(cat.name);
          setDescription(cat.description || "");
          setIsActive(cat.is_active);
          setExistingIcon(
            cat.icon_path
              ? `http://localhost:5000${cat.icon_path}`
              : ""
          );
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    if (file) formData.append("icon", file);   // only if a new file is chosen
  ...
  // -------------------------------------------------
  // UI (same look as AddCategoryForm)
  // -------------------------------------------------
  if (fetching) {
    return (
          ...
          {existingIcon && !file && (
            <div className="mb-3 flex items-center space-x-3">
              ...
              <span className="text-sm text-gray-600">
                Current icon (replace below if needed)
              </span>
            </div>
          )}
