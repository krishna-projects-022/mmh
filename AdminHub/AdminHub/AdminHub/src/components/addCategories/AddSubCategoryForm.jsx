// src/pages/AddSubCategoryForm.jsx
...
const AddSubCategoryForm = () => {
  ...
  // If you pass parent category id from the list page:
  const parentCategoryId = location.state?.categoryId;
  ...
  const handleSubmit = async (e) => {
    ...
    if (parentCategoryId) formData.append("category_id", parentCategoryId);

    setLoading(true);
    setError("");

    try {
      const res = await fetch("http://localhost:5000/api/subcategories/add", {
        method: "POST",
        body: formData,
      });
      ...
      if (result.success) {
        ...
        navigate(-1);  // or navigate to the list page you prefer
      } else {
        setError(result.message || "Failed to save sub-category");
      }
    } catch (err) {
      console.error(err);
